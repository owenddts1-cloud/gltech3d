...
do $$ begin perform cron.unschedule('refresh_sales_daily');
exception
when others then -- job não existia, ok
null;
end $$;
...
create or replace view public.v_sales_by_period as -- Semanal
select organization_id,
  'week'::text as grain,
...
select organization_id,
  'semester'::text,
  (
    date_trunc('year', day) + interval '6 months' * floor(
      (
        extract(
          month
          from day
...
do $$ begin perform cron.unschedule('refresh_print_costs_daily');
exception
when others then null;
end $$;
select cron.schedule(
    'refresh_print_costs_daily',
    '22 3 * * *',
    $$refresh materialized view concurrently public.mv_print_costs_daily $$
  );