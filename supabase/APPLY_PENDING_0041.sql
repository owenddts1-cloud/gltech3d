-- =============================================================================
-- APLICAR PENDÊNCIA — migration 0041_landing_cms
-- =============================================================================
-- Cole este arquivo INTEIRO no Supabase SQL Editor e clique em "Run".
-- Tudo é idempotente (safe to re-apply) e roda numa única transação — se algo
-- falhar, nada é aplicado (rollback automático).
--
-- Pré-requisito: 0030 (products) e 0037 (financial_records.platform) já aplicadas.
-- Depois de rodar, confira o resultado com as queries no fim do arquivo.
-- =============================================================================
...
-- =============================================================================
-- CONFERÊNCIA (rode depois do commit; deve bater com o esperado ao lado)
-- =============================================================================
-- 14 colunas novas em products:
--   select count(*) from information_schema.columns
--   where table_name = 'products'
--     and column_name in ('slug','is_published','is_top','bestseller_rank','sort_order',
--                         'hero_copy','price_range','links','videos','colors','material',
--                         'dimensions','stock_qty','sold_qty');   -- esperado: 14
--
-- 7 plataformas semeadas em 0% por org:
--   select organization_id, count(*) from public.platform_commissions
--   group by 1;                                                   -- esperado: 7 por org
--
-- RLS ligada nas duas tabelas novas:
--   select relname, relrowsecurity from pg_class
--   where relname in ('landing_settings','platform_commissions'); -- esperado: t, t
--
-- anon NÃO enxerga products (deve voltar 0 linhas):
--   select grantee, privilege_type from information_schema.role_table_grants
--   where table_name = 'products' and grantee = 'anon';
