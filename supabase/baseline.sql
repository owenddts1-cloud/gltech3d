...
-- ---- itens de O.S. + documentos imprimiveis (migration 0068) ----
...
-- ---- contador de vendas por peca: gatilho por delta (migration 0072) ----
-- O gatilho anterior (0055) so reagia a TRANSICAO de status, entao vincular uma
-- peca a uma venda ja paga nao contava nada, cancelar nunca descontava, mudar a
-- quantidade nao ajustava e apagar deixava o contador inflado. Agora o gatilho
-- calcula a CONTRIBUICAO antes e depois e aplica a diferenca, cobrindo os seis
-- casos. Inclui fn_reconcile_product_sales como rede de seguranca (so toca peca
-- com ao menos uma venda vinculada: sold_qty tambem e editavel a mao).
...
-- ---- products.model_source / model_license: origem do modelo, interno (migration 0073) ----
-- Vender a peca IMPRESSA e distribuir o ARQUIVO STL sao coisas juridicamente
-- diferentes. Esta coluna decide o que pode entrar num pack de arquivos, como
-- clausula de query em vez de julgamento repetido. Nasce 'desconhecido' em todas
-- as linhas: classificar o catalogo seria inventar dado juridico. Interna, fora
-- de PUBLIC_PRODUCT_COLUMNS. Aditiva e idempotente.
...
-- ---- model_versions: historico de edicao da peca (migration 0075) ----
...
-- ---- crm_pipelines: no maximo um default por org (migration 0076) ----
...
-- ---- products.model_id + proveniencia da estimativa (migration 0077) ----
...
-- ---- marketplace_orders.is_custom_item: peca avulsa (migration 0078) ----
...
-- ---- Instagram como canal, ao lado do WhatsApp (migration 0079) ----
...
-- ---- contacts: identidade do Instagram (migration 0080) ----
